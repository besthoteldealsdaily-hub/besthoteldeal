...
import { Inbox, Search, Phone, Mail } from "lucide-react";
import { LeadActions } from "./LeadActions";

export const metadata = { title: "DB Leads — Admin" };
...
export default async function AdminLeadsDBPage({
  searchParams,
}: {
  ...
  let query = db
    .from("leads")
    .select("id, hotel_name, hotel_slug, guest_name, guest_email, guest_phone, check_in, check_out, guests_count, budget_range, special_requests, source, status, forwarded_at, created_at")
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Guest inquiries for affiliate and lead-gen hotels. Forward to hotel partners manually.</p>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 mb-4 flex items-center gap-3 flex-wrap">
        ...
        <form className="flex-1 flex gap-2 flex-wrap" method="GET">
          ...
          >
            ...
            {["website", "whatsapp", "phone", "email", "partner"].map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-gray-100">
          <span className="text-xs text-gray-400">{leads?.length ?? 0} lead{leads?.length !== 1 ? "s" : ""}</span>
        </div>
      </div>
    </div>
  );
